...
10. **Shared component Tailwind classes missing in app**: Rex 0.20.0's Tailwind only scans `@source` paths that resolve **inside the app package root** (`packages/app`). An `@source` pointing at the sibling `packages/shared` (e.g. `@source "../../shared/src/components"` in `styles/globals.css`) is **silently ignored** — bare path, trailing slash, and `**/*.{ts,tsx}` glob all fail. So any utility class used *only* in a `packages/shared` component is never generated; classes that *also* appear in an app-scanned file (`pages/`, `components/`) work by coincidence, which masks the bug until a shared component introduces a class the app doesn't otherwise use (e.g. `xl:grid-cols-4`, `list-disc`, `pl-5`, a `hover:` color variant). **Workaround:** add the shared-only classes to the `@source inline("…")` safelist in `styles/globals.css` (already present). After editing a shared component's classes, audit them against the built CSS (`grep '\.<class>{' packages/app/.rex/build/client/globals-*.css`) and extend the safelist for any that are missing. Tracking upstream: https://github.com/limlabs/rex/issues/246 — remove the safelist once it's fixed. (Earlier guidance here claimed the trailing-slash `@source` form fixes this; it does not in 0.20.0.)
