 ...
 */
import { navigate, click, hover, scroll, screenshot } from '../../lib/client.js';
...
export default async function thisWeekMenuFlow(): Promise<void> {
  // Go to menus index
  await navigate('/menus#stage');
  await wait(4000);
  await screenshot('menu-01-index');

  // Hover then click on "Organic Week" menu
  await hover({ text: 'Organic Week' });
  await wait(400);
